#!/usr/bin/env python3
"""Build negative-sampling-demo.ipynb with proper multi-cell structure and executed outputs."""
...
# We'll capture stdout for each cell by redirecting sys.stdout
cells = []
...
def add_code_and_execute(source_lines, globals_dict):
    """Execute code, capture stdout, store both source and outputs."""
    source_text = '\n'.join(source_lines)
    ...
    try:
        exec(compile(source_text, '<cell>', 'exec'), globals_dict)
        exit_code = 0
    except Exception as e:
        print(f"Error: {e}", file=captured)
        exit_code = 1
    finally:
...
# ============================================================
add_markdown("""# CS224N L01 — Code Capsule: Negative Sampling vs Full Softmax

**Waypoint WP04**: Negative sampling vs full softmax

**Concept**: Full softmax in skip-gram requires computing P(o|c) over the ENTIRE vocabulary for every training step. Negative sampling (SGNS) replaces this with k binary classification problems.

**Official anchors**: notes §3.5 Eq.14-Eq.15; slides p35; R02 negative sampling paper

This notebook has 6 code cells you can run independently (after the setup cell).""")

# Shared globals for all code cells
g = {}
...
add_code_and_execute(setup_code, g)

# ============================================================
# Cell 2: Markdown - Part 1
# ============================================================
add_markdown("""## Part 1: Full Softmax — O(|V|) computation

Compute P(o|c) for ALL vocabulary words. The partition function Z requires |V| exp() calls.""")

# ============================================================
# Cell 3: Part 1 - Full Softmax
# ============================================================
part1_code = [
    'print("=" * 70)',
...
add_code_and_execute(part1_code, g)

# ============================================================
# Cell 4: Markdown - Part 2
# ============================================================
add_markdown("""## Part 2: Negative Sampling — O(k) computation

Instead of normalizing over all |V| words, SGNS uses k=5 negative samples + 1 positive pair. Each pair contributes a binary logistic loss.""")

# ============================================================
# Cell 5: Part 2 - Negative Sampling
# ============================================================
part2_code = [
    'print("=" * 70)',
...
add_code_and_execute(part2_code, g)

# ============================================================
# Cell 6: Markdown - Parts 3-4
# ============================================================
add_markdown("""## Part 3: Cost Comparison & Part 4: Gradient Direction

Compare computational cost, then analyze gradient directions (pull positive, push negatives).""")

# ============================================================
# Cell 7: Parts 3+4 - Cost + Gradient
# ============================================================
part34_code = [
    'print("=" * 70)',
...
add_code_and_execute(part34_code, g)

# ============================================================
# Cell 8: Markdown - Part 5
# ============================================================
add_markdown("""## Part 5: Visualizations

Generate plots showing: (1) full softmax probability distribution, (2) cost comparison, (3) gradient direction.""")

# ============================================================
# Cell 9: Part 5 - Plots + Results + Summary
# ============================================================
part5_code = [
    'print("=" * 70)',
    ...
    '""")',
    'print(f"Notebook execution completed at {datetime.now(timezone.utc).isoformat()}")',
]
add_code_and_execute(part5_code, g)

# ============================================================
# Build the notebook
# ============================================================
# Fix source lines: each line except last needs \n
for cell in cells:
    src = cell["source"]
...
output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "negative-sampling-demo.ipynb")
