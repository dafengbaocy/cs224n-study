...
python3 scripts/review_checks/check_links_headings.py --root . --paths Lectures/L01-word-vectors/code-capsules/one-hot-vs-dense.md Lectures/L01-word-vectors/00-concept-glossary.md --forbid-local-images && python3 scripts/review_checks/check_media_registry.py --root . --lecture L01-word-vectors --prefix code-capsule-one-hot-vs-dense --doc-path Lectures/L01-word-vectors/code-capsules/one-hot-vs-dense.md --require-doc-remotes-known --min-attempts-when-blocked 5 --min-gap-seconds-when-blocked 5 && python3 scripts/review_checks/check_code_capsule_run_log.py --root . --lecture L01-word-vectors --slug one-hot-vs-dense --task-id t_2397e6e3 && python3 scripts/review_checks/check_notebook_chinese.py --root . --paths Labs/L01-word-vectors/one-hot-vs-dense.ipynb && python3 - <<'PY'
...
if integration:
    ...
    if not slug:
        print('missing_code_capsule_slug_for_target=t_2397e6e3')
        raise SystemExit(2)
