...
**你是 CS224N paper-tutor，只处理 core reading L01-R01（Efficient Estimation of Word Representations in Vector Space）**。

### 输入

- `Lectures/L01-word-vectors/02-readings-map.md`（waypoint 清单 + core_readings；只读本篇 L01-R01 相关条目）
- `Sources/readings-canonical-lock.yaml`（L01-R01 canonical URL / PDF URL）
- `recovered/assets/papers/l01/L01-R01-mikolov-efficient-estimation-1301.3781.pdf`（本地 PDF）
- `recovered/evidence/l01/papers/L01-R01-*-extracted-text.txt`（抽取文本）

### 本篇焦点（来自 readings-map）

- 覆盖 waypoint：WP03, WP04
- 精读重点：Skip-gram/CBOW setup, center/context prediction, two vector matrices, softmax objective, and how the original word2vec paper maps onto slides p24-p35 and notes §3.2-§3.4
- 官方锚点：Schedule row (original word2vec paper); slides p24-p30; notes §3.2-§3.4; notes references lines 813-814

### 任务

1. 进入 L01-R01 原文 PDF，记录 canonical URL、PDF URL、sha256、提取方式。
2. 按 `02-readings-map.md` 本篇 waypoint 清单拆解 section / figure / equation。
3. **图片处理优先级（严格按此顺序）**：
   - **优先级 1**：查该图/表有无官方独立图片 URL（arXiv HTML 版、GitHub、作者主页）。有则直接在 paper note 中引用该 URL，记录 `来源：R01 {figure} 官方 URL：{url} 访问日期：2026-06-10`，不截图。
   - **优先级 2**：官方无独立 URL 时才截图存 `Assets/L01-word-vectors/papers/`。截图前先定位该页所有 figure/table 编号和标题，文件名必须含内容语义：
     - 格式：`L01-R01-p{page}-{content_slug}.png`
     - 例如：`L01-R01-p05-figure1-architecture.png`、`L01-R01-p07-table2-comparison.png`
     - 禁止只用页码（`p07.png`）
   ...
   - **媒体登记**：写入 `Lectures/L01-word-vectors/media-registry/paper-tutor-r01.json`，每张图一条：
     ```json
     {
       ...
       "items": [
         {
           "asset_id": "L01-R01-p05-table1-neg-vs-hs",
           "local_path": "Assets/L01-word-vectors/papers/L01-R01-p05-table1-neg-vs-hs.png",
           "remote_url": "https://...",
           "source_provenance": "L01-R01 PDF page 5 Table 1",
           "used_for": ["WP04 negative sampling efficiency"],
           "alt": "L01-R01 Table 1 negative sampling vs hierarchical softmax",
     ...
     ```
4. 不要用"至少抽一个图/表/公式"或"凑够 N 张"作为完成判据。扫完 L01-R01 所有 waypoint 相关页面，能帮理解的就抽。
5. 每个公式写：原始形式、符号解释、中文直觉、最小例子、对应 slides/notes 位置。
6. 写中文陪读：先人话，再技术名；回扣 slides/notes。
7. 输出 `Lectures/L01-word-vectors/03-paper-bridge.partial-r01.md`（R01 的 waypoint → 精确 wikilink 映射片段，供后续汇总）。跨文件 wikilink 必须用 Obsidian vault-root 路径：
   - 正确：`[[Papers/L01-word-vectors/L01-R01-efficient-estimation-word-representations-vector#Section 3 Eq 1|Eq(1)]]`
   - 错误：`[[../../Papers/.../L01-R01...#Section 3|Eq(1)]]`（不要 `../` 相对路径）
...
- `Papers/L01-word-vectors/L01-R01-efficient-estimation-word-representations-vector.md`
- `Assets/L01-word-vectors/papers/L01-R01-*.png`（截图文件名含语义）
- `Lectures/L01-word-vectors/media-registry/paper-tutor-r01.json`（local_path ↔ remote_url ↔ provenance 双轨登记）
- `Lectures/L01-word-vectors/03-paper-bridge.partial-r01.md`
- `artifacts/2026-06-10-paper-tutor-l01-r01.md`（执行报告 + 覆盖率自检表）
...
import fitz
pdf = Path("recovered/assets/papers/l01/L01-R01-mikolov-efficient-estimation-1301.3781.pdf")
assert pdf.exists(), f"missing required PDF for L01-R01: {pdf}"
doc = fitz.open(str(pdf))
assert doc.page_count > 0, f"unreadable PDF: {pdf}"
doc.close()
print("paper source asset ok:", str(pdf))
PY
test -f Lectures/L01-word-vectors/03-paper-bridge.partial-r01.md
/workspace/cs224n-study/.venv/bin/python scripts/review_checks/check_links_headings.py --root . --paths Papers/L01-word-vectors/L01-R01-efficient-estimation-word-representations-vector.md --forbid-local-images
/workspace/cs224n-study/.venv/bin/python scripts/review_checks/check_media_registry.py --root . --lecture L01-word-vectors --prefix paper-tutor-r01 --doc-path Papers/L01-word-vectors/L01-R01-efficient-estimation-word-representations-vector.md --doc-path Lectures/L01-word-vectors/03-paper-bridge.partial-r01.md --asset-path Assets/L01-word-vectors/papers/L01-R01-* --require-for-assets --require-doc-remotes-known --min-attempts-when-blocked 5 --min-gap-seconds-when-blocked 5
```

结果写到：
- `artifacts/2026-06-10-review-automation-v0-l01-paper-tutor-r01-source-evidence.jsonl`
- `artifacts/2026-06-10-review-automation-v0-l01-paper-tutor-r01-links-headings.jsonl`

### 禁止

- 不写 `00-课堂入口.md`
- 不处理其他 core reading（只处理 L01-R01）
